...
def signed_angle(v1: jnp.ndarray, v2: jnp.ndarray, plane_normal: jnp.ndarray, to_degree: bool = False) -> Union[jnp.ndarray, float]:
    ...
    sign = jnp.where(sign == 0, 1, sign)
        ...
        # Compute the unsigned angle using the JIT‑compatible `angle` function.
    # Note: We pass `plane_normal=look` and use the to_degree flag determined by `units`.
    unsigned_angle = angle(v1, v2, plane_normal=look, to_degree=(units == "deg"))