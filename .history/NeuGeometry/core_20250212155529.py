...
def robust_covariance_mest(
    ...
    def body_fn(state):
        ...
        # Check convergence based on the change in mean.
...
# Use static_argnums to mark parameters as static (positions 1,2,3,4,5).
@jit(static_argnums=(1, 2, 3, 4, 5))
def coord_eig_decomp(
    ...
    # Compute the eigendecomposition (using eigh, since the covariance matrix is symmetric).
    evals, evecs = jnp.linalg.eigh(cov)

    # Normalize eigenvalues to sum to 1 (i.e. fraction of variance explained) if PCA is desired.
    if PCA:
...
if __name__ == "__main__":
    ...
    # Compute the eigendecomposition using robust covariance estimation,
    # centering the data and normalizing eigenvalues for PCA.
    evals, evecs = coord_eig_decomp(coords, robust=True, center=True, PCA=True, sort=True, transpose=True)
