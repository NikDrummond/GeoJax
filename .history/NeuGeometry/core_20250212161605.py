...
@jit
def signed_angle(v1: jnp.ndarray,
                 v2: jnp.ndarray,
                 plane_normal: jnp.ndarray,
                 to_degree: bool = False) -> jnp.ndarray:
    """
...
@jit
def minimal_line_signed_angle(v1: jnp.ndarray,
                              v2: jnp.ndarray,
                              plane_normal: jnp.ndarray,
                              to_degree: bool = False) -> jnp.ndarray:
    """
    ...
    minimal_angle_rad = jnp.where(jnp.abs(angle_rad) > (jnp.pi / 2),
...
# =============================================================================
# Example usage (assuming reject, magnitude, etc. are defined):

if __name__ == "__main__":
    import jax.random as jr

    # Example input vectors (in 3D) and a plane normal.
    v1 = jnp.array([1.0, 0.0, 0.0])
    v2 = jnp.array([0.0, 1.0, 0.0])
    normal = jnp.array([0.0, 0.0, 1.0])

    # Compute the minimal signed angle in radians.
    theta_rad = minimal_line_signed_angle(v1, v2, normal, to_degree=False)
    # And in degrees.
    theta_deg = minimal_line_signed_angle(v1, v2, normal, to_degree=True)

    print("Minimal signed angle (radians):", theta_rad)
    print("Minimal signed angle (degrees):", theta_deg)
