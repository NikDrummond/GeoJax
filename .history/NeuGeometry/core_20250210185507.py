...
def project(vector, onto):
    ...
    if vector.ndim == 1:
        ...
        return scalar_projection(vector, onto=onto)[:, np.newaxis] * normalise(onto)
    else:
        raise ValueError('arr must be a single vector, or a stack of them!')
...
def signed_angle(v1, v2, plane_normal, units="deg"):
    """
    ...
    sign = np.array(np.sign(np.cross(v1, v2).dot(plane_normal)))

    # 0 means collinear: 0 or 180. Let's call that clockwise.
    sign[sign == 0] = 1

    return sign * angle(v1, v2, plane_normal, units=units)
