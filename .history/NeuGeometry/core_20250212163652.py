...
def rotate_coords(coords: jnp.ndarray, theta: float, axis: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    # Get the number of columns in the input coordinates.
    orig_dim = coords.shape[1]
    
    # Use a Python conditional (not lax.cond) since the shape is static.
    if orig_dim == 2:
        # Lift 2D coordinates to 3D by adding a zero z-component.
    ...
    # Rotate the coordinates (each row is a vector, so use R.T).
    rotated_3d = jnp.dot(coords_3d, R.T)
    
    # If the original coordinates were 2D, drop the z-component.
    if orig_dim == 2:
        rotated = rotated_3d[:, :2]
    else:
        rotated = rotated_3d
