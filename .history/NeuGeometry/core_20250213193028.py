...
def align_point_cloud(
    ...
    # Center the point cloud based on the provided conditions
    def center_on_point(c):
        return c - jnp.mean(c, axis=0) + center_coord if jnp.any(center_coord) else c - jnp.mean(c, axis=0)

    centered = lax.cond(center, center_on_point, lambda c: c, coords)
    ...
    # Re-center if needed based on the same logic as before
    aligned = lax.cond(center, lambda r: r - jnp.mean(r, axis=0) + center_coord if jnp.any(center_coord) else r - jnp.mean(r, axis=0), lambda r: r, rotated)
