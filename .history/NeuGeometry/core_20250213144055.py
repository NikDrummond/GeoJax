...
#                              robust: bool,
#                              center: bool | jnp.ndarray = False) -> jnp.ndarray:
...
#     if isinstance(center, bool):
#         rotated = rotated - (jnp.mean(rotated, axis = 0) if center else jnp.zeros(3))
#     return rotated
...
def align_point_cloud(coords: jnp.ndarray,
    ...
    robust : bool
        If True, use robust eigen decomposition.
    center : bool or jnp.ndarray, optional
        If a Boolean, indicates whether to center by subtracting the mean (True) or leave as is (False).
        Otherwise, it is used directly as the centering offset.
    ...
    """
    # --- Center the point cloud ---
    # Use lax.cond to branch on whether center is a scalar (i.e. a Boolean) or an array.
    centered = lax.cond(
        jnp.equal(jnp.ndim(center), 0),
        lambda _: lax.cond(center,
                           lambda _: coords - jnp.mean(coords, axis=0),
                           lambda _: coords,
                           operand=None),
    ...
    # (Assuming coord_eig_decomp is defined elsewhere.)
    _, eigvecs = coord_eig_decomp(centered, robust, False, False, True, True)
    ...
    # Optionally re-center the rotated point cloud.
    rotated = lax.cond(
        jnp.equal(jnp.ndim(center), 0),
        lambda _: lax.cond(center,
                           lambda _: rotated - jnp.mean(rotated, axis=0),
                           lambda _: rotated,
                           operand=None),
        lambda _: rotated,  # if center was an array, we assume it was already applied
        operand=None
    )
    return rotated
...
def circ_mean(samples, high=2*jnp.pi, low=0, axis=None, weights=None):
    ...
    # Map back to the original interval.
    return low + (mean_angle / (2*jnp.pi)) * period
...
def scale_coords(coords: jnp.ndarray, s) -> jnp.ndarray:
    ...
    if s.ndim == 1:
        ...
        assert s.shape[0] == coords.shape[1], "Length of s must equal number of coordinate columns."
    return coords * s
