...
def pairwise_euclidean(v1: Array, v2: Array) -> jnp.ndarray:
    ...
    if v1.shape[0] == v2.shape[0]:  # Case 1: Both 2D with matching rows
        return jnp.linalg.norm(v1 - v2, axis=1)
    elif v1.shape[0] == 1:  # Case 2: v1 is a single point, v2 is multiple points
        return jnp.linalg.norm(v2 - v1, axis=1)
    elif v2.shape[0] == 1:  # Case 3: v2 is a single point, v1 is multiple points
        return jnp.linalg.norm(v1 - v2, axis=1)
    else:
