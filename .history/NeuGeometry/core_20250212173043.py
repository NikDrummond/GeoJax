...
@jit
def align_point_cloud(coords: jnp.ndarray, order: jnp.ndarray, target_basis: jnp.ndarray, robust: bool, center) -> jnp.ndarray:
    """
    Rotate the point cloud so that its eigenvectors (obtained from the covariance matrix)
    align with a specified target basis. The inputs are:
      - coords: an (N×n) array of points (n = 2 or 3)
      - order: an integer array of length n specifying the desired ordering of eigenvectors.
      - target_basis: an (n×n) array whose rows are the desired target unit vectors.
      - robust: a Boolean indicating whether to use robust covariance estimation.
      - center: either a Boolean or an array. If a Boolean: True subtracts the mean,
                False does nothing. If an array, subtract that array from each coordinate.
    The function rotates the centered point cloud (using the minimal rotation) so that the eigenvector
    in slot i (after reordering and sign–correction) aligns with target_basis[i]. No mirroring is introduced.
    """
    # Ensure robust and center are JAX arrays.
    robust = jnp.asarray(robust)
    center = jnp.asarray(center)

    # --- Center the coordinates ---
    # If center is a scalar (0D), we assume it is a Boolean.
    # If it is True, subtract the mean; if False, do nothing.
    # Otherwise (if center.ndim > 0), subtract center from each coordinate.
    centered = lax.cond(
    ...
    # --- Compute covariance (robust or not) and eigen decomposition ---
    # Here we call coord_eig_decomp with center=False (since we already centered),
    # PCA=False (we only need the eigenvectors), sort=True, transpose=True.
    _, eigvecs = coord_eig_decomp(centered, robust, False, False, True, True)
    # eigvecs is now an (n×n) array with eigenvectors as rows.
    
    # --- Reorder eigenvectors ---
    E_ordered = jnp.take(eigvecs, order, axis=0)

    # --- Adjust signs to avoid large rotations ---
    # For each eigenvector (each row), if its dot product with the corresponding target vector is negative,
    # flip its sign.
    dots = jnp.sum(E_ordered * target_basis, axis=1, keepdims=True)
    sign_factor = jnp.where(dots < 0, -1.0, 1.0)
    E_ordered_adjusted = E_ordered * sign_factor

    # --- Compute the rotation matrix ---
    # The minimal rotation mapping the computed (ordered, sign–corrected) eigenbasis onto the target basis is:
    #       R = target_basis^T @ E_ordered_adjusted
    # (When applying to row–vectors, we use R^T.)
