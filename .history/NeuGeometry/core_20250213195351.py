...
def align_point_cloud(
    ...
    # Conditionally center the point cloud based on the provided arguments
    centered = lax.cond(
        center,
    ...
    # Apply reordering to the eigenvectors
    E = eigvecs[order]

    # Adjust signs to align with the target basis
    signs = jnp.sign(jnp.sum(E * target_basis, axis=1))
    E_adjusted = E * signs[:, None]

    # Compute rotation matrix using pseudo-inverse for non-orthogonal bases
    R_total = jnp.matmul(jnp.linalg.pinv(target_basis), E_adjusted)
