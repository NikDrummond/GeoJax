...
def _validate_vector(x, ndim, shape=None):
    x = jnp.asarray(x)
    if x.ndim != ndim:
        raise ValueError(f"Expected {ndim}D input, got {x.ndim}D")
    if shape and x.shape[-1] != shape:
        raise ValueError(f"Expected last dimension of size {shape}, got {x.shape[-1]}")
    return x
...
def normalise(x):
    x = _validate_vector(x, ndim=1 if x.ndim == 1 else 2, shape=3)
    return _normalise(x)
...
def pairwise_euclidean(x, y):
    x = _validate_vector(x, ndim=2, shape=3)
    y = _validate_vector(y, ndim=2, shape=3)
    return _pairwise_euclidean(x, y)

@jit
def _reject(u, v):
    proj = (jnp.sum(u * v, axis=-1, keepdims=True) / jnp.sum(v * v, axis=-1, keepdims=True)) * v
    return u - proj

def reject(u, v):
    u = _validate_vector(u, ndim=1 if u.ndim == 1 else 2, shape=3)
    v = _validate_vector(v, ndim=1 if v.ndim == 1 else 2, shape=3)
    return _reject(u, v)

# Next: angle and signed_angle will follow the same pattern
