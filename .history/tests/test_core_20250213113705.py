...
def test_single_vector_with_multiple_points():
    ...
    result_v1 = pairwise_euclidean(single_vector, multiple_points)
    jnp.testing.assert_allclose(result_v1, expected_v1)

    # Expected results when single_vector is v2
    expected_v2 = jnp.array([jnp.sqrt(8.0), jnp.sqrt(32.0)])
    result_v2 = pairwise_euclidean(multiple_points, single_vector)
    jnp.testing.assert_allclose(result_v2, expected_v2)