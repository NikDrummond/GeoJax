...
def test_project_to_plane_custom_normal():
    ...
    assert jnp.allclose(
        projected, jnp.array([[1.0, 2.0]]), atol=1e-5
    ), f"Got {projected}"
...
def test_mercator_projection_basic():
    ...
    lon, merc_y = result[0]
    assert jnp.isclose(
        merc_y, 0.0, atol=1e-5
    ), f"Mercator Y should be zero, got {merc_y}"
    assert jnp.isclose(lon, 0.0, atol=1e-5), f"Longitude should be zero, got {lon}"
...
def test_lambert_azimuthal_projection_basic():
    ...
    assert jnp.allclose(
        result, jnp.array([[0.0, 0.0]]), atol=1e-5
    ), f"Expected origin, got {result}"
...
def test_mercator_projection_handles_poles():
    ...
    assert jnp.isfinite(
        merc_y
    ), f"Expected finite Mercator Y at near-pole, got {merc_y}"
...
def test_lambert_projection_scale():
    ...
    assert jnp.all(
        jnp.isfinite(result)
    ), f"Expected finite Lambert projection, got {result}"
