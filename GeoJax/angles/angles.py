...
from ..core.core import dot, cross, reject, magnitude
...
def angle(
    ...
    plane_normal : jnp.ndarray, optional
        Normal vector of plane to project onto. If None, computes angle in full space.
        Default is None.
    to_degree : bool, optional
        If True, return angle in degrees; otherwise radians. Default is False.
    assume_normalized : bool, optional
        If True, assumes vectors are already normalized. Default is False.

    Returns
    -------
    jnp.ndarray
        Angle(s) between vectors, shape (...,).
    """
...
def angle_between_planes(n1: jnp.ndarray, n2: jnp.ndarray, to_degree: bool = False) -> jnp.ndarray:
    ...
    ----------
    n1 : jnp.ndarray
        Normal vector of first plane, shape (..., D).
    n2 : jnp.ndarray
        Normal vector of second plane, shape (..., D), broadcastable with n1.
    to_degree : bool, optional
        If True, return angle in degrees; otherwise radians. Default is False.
    ...
    -------
    jnp.ndarray
        Angle between planes in range [0, π/2] (or [0°, 90°]), shape (...,).
    """
    cos_theta = dot(n1, n2)
...
def minimum_signed_angle(
    ...
    Returns the angle closest to zero, wrapping angles outside [-π/2, π/2].
    ...
    v2 : jnp.ndarray
        Second vector(s) of shape (..., D), broadcastable with v1.
    plane_normal : jnp.ndarray
        Normal vector defining the plane, shape (D,) or broadcastable.
    to_degree : bool, optional
        If True, return angle in degrees; otherwise radians. Default is False.
    ...
    -------
    jnp.ndarray
        Minimal signed angle(s) in range [-π/2, π/2] (or [-90°, 90°]), shape (...,).
    """
