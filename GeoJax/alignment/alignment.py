...
def robust_covariance_mest(
    ...
    c : float, optional
        Tuning constant for robust estimation. Default is 1.5.
    tol : float, optional
    ...
    -------
    jnp.ndarray
        Robust covariance matrix of shape (D, D).
    """
    n, d = X.shape
...
def coord_eig_decomp(
    ...
    robust : bool, optional
        If True, use robust covariance estimation. Default is True.
    center : bool, optional
        If True, center coordinates before decomposition. Default is False.
    PCA : bool, optional
        If True, normalize eigenvalues to sum to 1 (proportions). Default is True.
    sort : bool, optional
        If True, sort eigenvalues in descending order. Default is True.
    transpose : bool, optional
        If True, transpose eigenvector matrix. Default is True.

    Returns
    -------
    tuple[jnp.ndarray, jnp.ndarray]
        Eigenvalues and eigenvectors. Eigenvalues shape (D,), eigenvectors shape (D, D).
    """
...
def minimum_theta(R: jnp.ndarray) -> jnp.ndarray:
    ...
    -------
    jnp.ndarray
        Minimum rotation angle in radians (scalar).
    """
    trace = jnp.trace(R)
...
def alignment_matrix(from_basis: jnp.ndarray, to_basis: jnp.ndarray) -> jnp.ndarray:
    ...
    from_basis : jnp.ndarray
        Source basis vectors, shape (D, D).
    to_basis : jnp.ndarray
        Target basis vectors, shape (D, D).

    Returns
    -------
    jnp.ndarray
        Rotation matrix of shape (D, D) such that R @ from_basis ≈ to_basis.
    """
